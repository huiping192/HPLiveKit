...
HPLiveKit 由两个 target 组成：
- **HPLiveKitCore** — 跨平台核心库（编码、推流、数据模型），可在 macOS 上直接构建和测试
- **HPLiveKit** — iOS facade（采集层 + LiveSession），依赖 UIKit，需要 xcodebuild

### HPLiveKitCore — macOS 直接构建/测试（推荐用于核心逻辑开发）
```bash
# 构建核心库（macOS）
swift build --target HPLiveKitCore

# 运行核心测试（macOS，无需模拟器）
swift test --filter HPLiveKitCoreTests
```
